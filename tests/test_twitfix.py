...
@pytest.mark.asyncio
class RetrieveTweetsTestCase:
    async def test_text_tweet_extract(self):
        tweet = await extract_status(testTextTweet)
        ...
        assert not tweet["is_quote_status"]

    async def test_video_tweet_extract(self):
        tweet = await extract_status(testVideoTweet)
        ...
        assert (
            video["media_url_https"]
            == "https://pbs.twimg.com/media/EYeX7akWsAIP1_1.jpg"
        )
        assert video["type"] == "video"
        assert not tweet["is_quote_status"]

    async def test_media_tweet_extract(self):
        tweet = await extract_status(testMediaTweet)
    ...
    async def test_multimedia_tweet_extract(self):
        tweet = await extract_status(testMultiMediaTweet)
    ...
    async def test_poll_tweet_extract(self):
        tweet = await extract_status(
...
class QRTweetTestCase:
    ...
    async def test_qrt_video_tweet(self):
        cache.clear_cache()
        # this is an incredibly lazy test, todo: improve it in the future
        resp = await client.get(
...
@pytest.mark.asyncio
class EmbedTestCase:
    def test_embed_from_scratch(self):
        cache.clear_cache()
        client.get(
            testTextTweet.replace("https://twitter.com", ""),
            headers={"User-Agent": "test"},
        )
        client.get(
    ...
    async def test_very_long_embed(self):
        cache.clear_cache()
        cache.set_cache(
            {
    ...
    async def test_embed_from_outdated_cache(self):
    ...
    async def test_direct_embed(self):
        resp = await client.get(
...
async def test_message404():
    ...
    logger.debug(await resp.data)
    assert resp.status_code == 200
    assert constants.TWEET_NOT_FOUND in str(await resp.data)
